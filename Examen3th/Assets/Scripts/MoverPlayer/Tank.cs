...
public class Tank : MonoBehaviour
{
    ...
    async void Move()
    {
        ...
        {
            ts.demonio.SetTile(_pathNodes[i], playerSprite);
            ts.demonio.SetTransformMatrix(_pathNodes[i], Matrix4x4.TRS(offset, Quaternion.Euler(0, 0, 0), Vector3.one));
            await Task.Delay(1000);
            if (i != 0)
            {
                ts.demonio.ClearAllTiles();
            }
        }
    }
